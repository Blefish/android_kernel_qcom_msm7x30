		...
		unsigned gpio, int value)
{
	...
	if (rc)
		pr_err("FAIL pm8xxx_writeb(): rc=%d. (gpio=%d, value=%d)\n",
		       rc, gpio, value);
...
static void pm_gpio_write(struct gpio_chip *gpio_chip, unsigned offset, int val)
{
	struct pm_gpio_chip *pm_gpio_chip = dev_get_drvdata(gpio_chip->dev);
	pm_gpio_set(pm_gpio_chip, offset, val);
}

static int pm_gpio_direction_input(struct gpio_chip *gpio_chip, unsigned offset)
{
	struct pm_gpio_chip *pm_gpio_chip = dev_get_drvdata(gpio_chip->dev);
...
static void pm_gpio_dbg_show(struct seq_file *s, struct gpio_chip *gpio_chip)
{
	...
	for (i = 0; i < gpio_chip->ngpio; i++) {
		...
		seq_printf(s, "gpio-%-3d (%-12.12s) %-10.10s %s",
			   gpio_chip->base + i,
			   label ? label : "--",
			   cmode[mode],
			   state ? "hi" : "lo");
...
static int pm_gpio_probe(struct platform_device *pdev)
{
	...
	int ngpios, gpio_base;

	ret = of_property_read_u32(node, "ngpios", &ngpios);
	if (ret) {
		pr_err("failed to read 'ngpios' ret=%d\n", ret);
		return ret;
	}

	ret = of_property_read_u32(node, "gpio-base", &gpio_base);
	if (ret) {
		pr_err("failed to read 'gpio-base' ret=%d\n", ret);
		return ret;
	}

	pm_gpio_chip = devm_kzalloc(dev, sizeof(*pm_gpio_chip), GFP_KERNEL);
	if (!pm_gpio_chip) {
		pr_err("Cannot allocate pm_gpio_chip\n");
		return -ENOMEM;
	}

	pm_gpio_chip->bank1 = devm_kzalloc(dev, sizeof(u8) * ngpios,
					   GFP_KERNEL);
	...
	pm_gpio_chip->gpio_chip.ngpio = ngpios;
	pm_gpio_chip->gpio_chip.can_sleep = 0;
	pm_gpio_chip->gpio_chip.dev = &pdev->dev;
	pm_gpio_chip->gpio_chip.base = gpio_base;
...
{
	struct pm_gpio_chip *pm_gpio_chip = platform_get_drvdata(pdev);
...
static struct of_device_id pm_gpio_of_match_table[] = {
	{ .compatible = "qcom,pm8058-gpio", },
	{ /* end of table */ }
};
MODULE_DEVICE_TABLE(of, pm_gpio_of_match_table);
...
static struct platform_driver pm_gpio_driver = {
	...
	.driver		= {
		...
		.owner		= THIS_MODULE,
		.of_match_table	= pm_gpio_of_match_table,
	},
};
