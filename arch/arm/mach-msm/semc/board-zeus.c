...
#define MSM_FB_OVERLAY0_WRITEBACK_SIZE (864 * 480 * 3 * 2)
#else
...
static struct msm_gpio synaptics_gpio_config_data[] = {
	{ GPIO_CFG(SYNAPTICS_TOUCHPAD_GPIO, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP,
		GPIO_CFG_2MA), "synaptics_touchpad_irq" },
};
...
#ifdef CONFIG_INPUT_GP2A
static uint32_t gp2a_gpio_config =
	GPIO_CFG(GP2A_GPIO, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA);

static int gp2a_gpio_setup(struct i2c_client *client)
{
	int rc;

	rc = gpio_tlmm_config(gp2a_gpio_config, GPIO_CFG_ENABLE);
	if (rc)
		pr_err("%s: gpio_tlmm_config(%#x)=%d\n",
			__func__, gp2a_gpio_config, rc);

	return rc;
}

static int gp2a_gpio_shutdown(struct i2c_client *client)
{
	int rc;

	rc = gpio_tlmm_config(gp2a_gpio_config, GPIO_CFG_DISABLE);
	if (rc)
		pr_err("%s: gpio_tlmm_config(%#x)=%d\n",
			__func__, gp2a_gpio_config, rc);
...
static struct gp2a_platform_data gp2a_platform_data = {
	...
	.hw_shutdown = gp2a_gpio_shutdown,
};
#endif
